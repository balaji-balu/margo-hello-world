...
func (_q *SiteQuery) QueryHosts() *HostQuery {
...
func (_q *SiteQuery) WithHosts(opts ...func(*HostQuery)) *SiteQuery {
	query := (&HostClient{config: _q.config}).Query()
...
func (_q *SiteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Site, error) {
	...
	if query := _q.withHosts; query != nil {
		if err := _q.loadHosts(ctx, query, nodes,
			func(n *Site) { n.Edges.Hosts = []*Host{} },
			func(n *Site, e *Host) { n.Edges.Hosts = append(n.Edges.Hosts, e) }); err != nil {
			return nil, err
		}
	}
